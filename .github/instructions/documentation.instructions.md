...
flowchart LR
    %% Flow A
    ST(("Start")) --> IN[/User Input/]
    IN -->|Arrow Label| DB[(Database)]
    DB -.-> FX[Process]
    subgraph SectionA
      FX --> IF{Condition}
      IF ==>|NO?| HEX{{Prepration}}
      HEX --> IF
      IF ==>|YES?| RIBON>Label]
    end
    RIBON StreamConnection@--> Files@{shape: docs, label: "Multiple Documents"}
    Files --> END(((FINISH)))

    %% Flow B
    ALT((Another Process)) --o PRE[[Predefined Process]]
    subgraph SectionB
      PRE --- LMT[/Limiting\]
      LMT --x FLT[\FILTER/]
      FLT ---->|Longer Line| x[\This/]
    end

    SectionA <--> SectionB

    %% Classes
    SectionA:::light_class
    ALT:::red_class

    %% Styling
    classDef light_class fill:#eee,stroke:#333,stroke-width:2px,color:#000
    classDef red_class fill:#f88,stroke:#FFF,stroke-width:2px,color:#000,stroke-dasharray:5

    %% Animate
    StreamConnection@{ animation: fast } 
```

This diagram shows the high-level flow of a system.

**Formatting tips:**
- Use clear, descriptive node names.
- Use arrows to indicate direction and relationships.
- Use subgraphs to group related nodes.
- Use classes to style nodes and edges for better visibility.
- use proper shapes for different types of nodes (e.g., rectangles for processes, diamonds for decisions).
- Use indentation, line breaks and comments for readability.
- write the diagram in clean sections like flow, classes, styling, and animation.
